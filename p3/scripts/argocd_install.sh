...
elapsed_time=0
while [[ $(kubectl get pods -n dev -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}') != "True" ]]; do
  if [[ $elapsed_time -ge $timeout ]]; then
    echo "[ERROR][DEV] Timeout reached while waiting for all pods to be ready."
    exit 1
  fi
  echo "[INFO][DEV] Waiting all pods to be ready..."
  sleep 10
